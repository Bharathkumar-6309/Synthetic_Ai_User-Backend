...
class ModelRouter:
    ...
    def candidates(self) -> list[ModelCandidate]:
        """
        Returns an ordered list of models to attempt. Gemini is tried first
        only if an API key is configured; Ollama is always listed as a
        fallback (llm_client will simply fail fast if it's unreachable).
        """
    ...
    def has_any_llm_configured(self) -> bool:
        ...
        provider (Gemini key present) vs. relying purely on a local Ollama
